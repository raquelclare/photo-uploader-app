...
// ========== Implementing the methods for creating the random filenames and the output stream for writing to the files
...
AvatarStorage.prototype._generateRandomFilename = function() {
	...
	// using crypto to create a random md5 hash to use as filename and...
	var checksum = crypto.createHash('MD5').update(bytes).digest('hex');

	// return as filename the hash with the output extension
	// ...append the output from the options as the file extension
...
AvatarStorage.prototype._createOutputStream = function(filepath, cb) {
	...
	// create a writeable stream from the filepath
	var output = fs.createWriteStream(filepath);

	// callback function is required, since we are using it on the stream event handlers
...
// ======== Implementing the _processImage() method that does the actual image processing ==========
...
AvatarStorage.prototype._processImage = function(image, cb) {
	...
	if (threshold && square > threshold) {
		clone = (square == width) ? clone.resize(threshold, Jimp.AUTO) : clone.resive(Jimp.AUTO, threshold);
	}
	...
	// if responsive is enabled, the image is clones and scaled for each of the responsize sizes and then an output
	// stream is created using the _createOutputStream() method for each image file of the respective sizes
	// the filename for eaxh size take the format [random_filename_hash]_[size].[output_extension]
	if (this.options.responsive) {

		// map throught he responsive sizes and push them to the batch
		batch = _.map(sizes, function(size) {
			...
			// create the complete filepath and create a writeable stream for it
		...
		// if responsive is disabled, then only the current image and an output stream for it is put in a batch
		// for processing
	} else {
	...
	// each item in the batch is processed by converting the Jimp image buffer into a readable stream using
	// streamifier and then piping the readable stream to the output stream
	_.each(batch, function(current) {
		// get the buffer of the Jimp using the outpur mime type
...
AvatarStorage.prototype._handleFile = function(req, file, cb) {
	...
	// create a writeable stream using concat-stream that will
